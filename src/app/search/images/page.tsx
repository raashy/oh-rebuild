import { ImageResult, VideoResult as VideoResultType } from "@/actions/brave/types/results";
...
import ImageResultsContainer from "@/components/ImageResults/ImageResultsContainer";
...
export default async ({ searchParams }: { searchParams: { q: string; page: string } }) => {
  ...
  const response = await fetch(`https://api.search.brave.com/res/v1/images/search?q=${searchParams.q}&offset=${searchParams.page || 0}`, {
    headers: {
  ...
  return (
    <Suspense fallback={<Loading />} key={searchParams.q}>
      <section className="pl-[16.5rem] pr-[6rem] py-8">
        <div className="flex max-w-[70rem] flex-col gap-6">
          <ImageResultsContainer results={results} />

          <div className="flex gap-3 justify-between w-full">
            {previousPage || previousPage === 0 ? (
              <a
                href={`/search/images?q=${searchParams.q}&page=${previousPage}`}
            ...
            {nextPage ? (
              <a
                href={`/search/images?q=${searchParams.q}&page=${nextPage}`}
