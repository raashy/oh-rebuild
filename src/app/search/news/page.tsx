...
import NewsResult from "@/components/NewsResults/NewsResult";
import { ArrowLeft, ArrowRight } from "lucide-react";
import { redirect } from "next/navigation";
import { Suspense } from "react";
import Loading from "./loading";
import NewsResultSkeleton from "@/components/NewsResults/NewsResultSkeleton";
...
export default async ({ searchParams }: { searchParams: { q: string; page: string } }) => {
  ...
  const response = await fetch(`https://api.search.brave.com/res/v1/news/search?q=${searchParams.q}&offset=${searchParams.page || 0}`, {
    headers: {
  ...
  const results = data.results as NewsResultType[];
        ...
        <div className="flex max-w-[35rem] flex-col gap-6">
          ...
          <div className="flex gap-3 justify-between w-full">
            {previousPage || previousPage === 0 ? (
              <a
                href={`/search/news?q=${searchParams.q}&page=${previousPage}`}
            ...
            {nextPage ? (
              <a
                href={`/search/news?q=${searchParams.q}&page=${nextPage}`}
